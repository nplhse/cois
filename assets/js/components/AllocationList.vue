    ...
    <div>
        ...
        <b-sidebar id="sidebar-1" title="Filters" shadow>
            ...
            <div class="px-3 py-2">
                ...
                <b-card title="by Properties" class="mb-2">
                    <b-form-checkbox
                        v-model="filters.properties.requiresResus"
                        name="check-button"
                        switch
                    >
                        &nbsp;Requires Resus (S+)
                    ...
                    <b-form-checkbox
                        v-model="filters.properties.requiresCathlab"
                        name="check-button"
                        switch
                    >
                        &nbsp;Requires Cathlab (H+)
                    ...
                    <b-form-checkbox
                        v-model="filters.properties.isWithPhysician"
                        name="check-button"
                        switch
                    >
                        &nbsp;Is with Physician (N+)
                    ...
                    <b-form-checkbox
                        v-model="filters.properties.isCPR"
                        name="check-button"
                        switch
                    >
                        &nbsp;Is CPR (R+)
                    ...
                    <b-form-checkbox
                        v-model="filters.properties.isVentilated"
                        name="check-button"
                        switch
                    >
                        &nbsp;Is Ventilated (B+)
                    ...
                    <b-form-checkbox
                        v-model="filters.properties.isShock"
                        name="check-button"
                        switch
                    >
                        &nbsp;Is Shock
                    ...
                    <b-form-checkbox
                        v-model="filters.properties.isPregnant"
                        name="check-button"
                        switch
                    >
                        &nbsp;Is Pregnant
                    ...
                    <b-form-checkbox
                        v-model="filters.properties.isWorkAccident"
                        name="check-button"
                        switch
                    >
                        &nbsp;Is Work Accident (BG+)
        ...
        >
            ...
            <template #cell(id)="data">
                <b
                    ><a :href="data.item.id.toString()">{{ data.value }}</a></b
                >
            </template>

            <template #cell(hospital)="data">
                <a :href="hospitalLinks[data.item.hospital].toString()">{{
                    hospitals[data.item.hospital]
                }}</a>
            </template>
            ...
            <template #cell(dispatch)="data">
                <b>{{ data.item.speciality }}</b
                ><br />
                {{ data.item.rMI }} {{ data.item.pZCText }}
            </template>

            <template #cell(properties)="data">
                <b v-if="data.item.requiresResus" class="text-danger">S+</b>
                <b v-if="data.item.requiresCathlab" class="text-danger">H+ </b>
                <b v-if="data.item.isWithPhysician" class="text-danger">N+ </b>
                <b v-if="data.item.isCPR" class="text-danger">R+ </b>
                <b v-if="data.item.isVentilated" class="text-danger">B+ </b>
                <b v-if="data.item.isPregnant" class="text-warning">Pregnant</b>
                <b v-if="data.item.isWorkAccident" class="text-danger">BG+ </b>
            </template>
            ...
            <template #row-details="row">
                <b-card>
                    <b-row class="mb-3">
                        <b-col
                            ><b>Secondary RMI</b><br />{{
                                row.item.secondaryPZC
                            }}</b-col
                        >
                        <b-col
                            ><b>Secondary Diagnosis</b><br />{{
                                row.item.secondaryPZCText
                            }}</b-col
                        >
                        <b-col
                            ><b>Speciality detail</b><br />{{
                                row.item.specialityDetail
                            }}</b-col
                        >
                        <b-col>
                            ...
                            <template v-else>
                                <p class="text-success">No (Open)</p>
                            </template>
                        </b-col>
                    </b-row>
                    ...
                    <b-row>
                        ...
                        <b-col
                            ><b>Handover point</b><br />{{
                                row.item.handoverPoint
                            }}</b-col
                        >
                        <b-col
                            ><b>Mode of transport</b><br />{{
                                row.item.modeOfTransport
                            }}</b-col
                        >
                        <b-col
                            ><b>Transport comment</b><br /><i>{{
                                row.item.comment
                            }}</i></b-col
                        >
                    </b-row>
        ...
        <b-row>
            <b-col class="pb-2">
                <div>
                    Showing <b>{{ perPage }}</b> of
                    <b>{{ totalItems }}</b> allocations. Sorting By:
                    <b>{{ sortBy }}</b
                    >, Sort Direction:
        ...
        <b-row>
            <b-col>
                <b-form-group>
                    <label class="mr-sm-2" for="pagination-selector"
...
export default {
    ...
    methods: {
        async loadAllocations() {
            ...
            let response;
            try {
                response = await fetchAllocations(
                    this.currentPage,
                    this.perPage,
                    this.sortBy,
                    this.sortDesc,
